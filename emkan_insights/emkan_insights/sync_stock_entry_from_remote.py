...
REMOTE_DT = "Stock Entry"
LOCAL_DT = "External Stock Entry"
PROGRESS_KEY_PREFIX = "se_sync_progress"


def _prepare_stock_entry_for_external_sync(item, company_abbr):
    item = dict(item or {})
    ...
    # Store the prefixed remote name as remote_id for deduplication lookups.
    # Do NOT set item["name"] — naming is now handled by the EXT-STE-.YYYY.- series.
    if prefixed_remote_id:
        item["remote_id"] = prefixed_remote_id

    # Always set naming_series so Frappe uses our custom series
    item["naming_series"] = "EXT-STE-.YYYY.-"

    # Remove name/id overrides so Frappe auto-generates the name from the series
    item.pop("name", None)
    item.pop("id", None)
...
@frappe.whitelist()
def sync_stock_entries_from_remote(site_url, api_key, api_secret, ref_doctype, child_docname, company=None, configuration_name=None):
    """
    ...
    if not _acquire_sync_lock(child_docname):
        frappe.throw(_("A Stock Entry sync is already running for this row. Please wait for it to finish."))
    ...
    try:
        ...
        for idx, chunk in enumerate(chunks):
            frappe.enqueue(
                "emkan_insights.emkan_insights.sync_stock_entry_from_remote.process_stock_entry_chunk",
                queue="long",
                timeout=900,
                job_name=f"se_sync_chunk_{child_docname}_{idx}",
                site_url=site_url,
                api_key=api_key,
        ...
        return f"Queued {len(chunks)} chunk(s) covering {len(names)} Stock Entries"
...
@frappe.whitelist()
def process_stock_entry_chunk(site_url, api_key, api_secret, company, company_abbr, configuration_name, child_docname, remote_names, is_last_chunk=False):
    """
    Background worker for a single chunk of Stock Entry names.
    ...
    try:
        ...
        docs = [_prepare_stock_entry_for_external_sync(item, company_abbr) for item in docs]
