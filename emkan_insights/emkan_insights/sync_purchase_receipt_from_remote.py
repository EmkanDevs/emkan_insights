...
REMOTE_DT = "Purchase Receipt"
LOCAL_DT = "External Purchase Receipt"
PROGRESS_KEY_PREFIX = "pr_sync_progress"


def _prepare_purchase_receipt_for_external_sync(item, company_abbr):
    item = dict(item or {})
...
@frappe.whitelist()
def sync_purchase_receipts_from_remote(site_url, api_key, api_secret, ref_doctype, child_docname, company=None, configuration_name=None):
    """
    Lightweight entry point. Only lists remote names, splits into chunks,
    and enqueues one background job per chunk.
    """
    if not _acquire_sync_lock(child_docname):
        frappe.throw(_("A Purchase Receipt sync is already running for this row. Please wait for it to finish."))
    ...
    try:
        ...
        for idx, chunk in enumerate(chunks):
            frappe.enqueue(
                "emkan_insights.emkan_insights.sync_purchase_receipt_from_remote.process_purchase_receipt_chunk",
                queue="long",
                timeout=900,
                job_name=f"pr_sync_chunk_{child_docname}_{idx}",
                site_url=site_url,
                api_key=api_key,
        ...
        return f"Queued {len(chunks)} chunk(s) covering {len(names)} Purchase Receipts"
...
@frappe.whitelist()
def process_purchase_receipt_chunk(site_url, api_key, api_secret, company, company_abbr, configuration_name, child_docname, remote_names, is_last_chunk=False):
    """
    Background worker for a single chunk of Purchase Receipt names.
    """
    base_url = (site_url or "").rstrip("/")
    headers = {
        "Authorization": f"token {api_key}:{api_secret}",
    ...
    try:
        ...
        docs = [_prepare_purchase_receipt_for_external_sync(item, company_abbr) for item in docs]
        ...
        if is_last_chunk:
            ...
            frappe.publish_realtime(
                "purchase_receipt_sync_complete",
                {
                    "child_docname": child_docname,
