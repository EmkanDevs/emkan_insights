...
REMOTE_DT = "Payment Request"
LOCAL_DT = "External Payment Request"
PROGRESS_KEY_PREFIX = "prq_sync_progress"


def _prepare_payment_request_for_external_sync(item, company_abbr):
    item = dict(item or {})
...
@frappe.whitelist()
def sync_payment_requests_from_remote(site_url, api_key, api_secret, ref_doctype, child_docname, company=None, configuration_name=None):
    if not _acquire_sync_lock(child_docname):
        frappe.throw(_("A Payment Request sync is already running for this row. Please wait for it to finish."))
    ...
    try:
        ...
        for idx, chunk in enumerate(chunks):
            frappe.enqueue(
                "emkan_insights.emkan_insights.sync_payment_request_from_remote.process_payment_request_chunk",
                queue="long",
                timeout=900,
                job_name=f"prq_sync_chunk_{child_docname}_{idx}",
                site_url=site_url,
                api_key=api_key,
        ...
        return f"Queued {len(chunks)} chunk(s) covering {len(names)} Payment Requests"
...
@frappe.whitelist()
def process_payment_request_chunk(site_url, api_key, api_secret, company, company_abbr, configuration_name, child_docname, remote_names, is_last_chunk=False):
    base_url = (site_url or "").rstrip("/")
    headers = {
        "Authorization": f"token {api_key}:{api_secret}",
        "Content-Type": "application/json",
    ...
    try:
        ...
        docs = [_prepare_payment_request_for_external_sync(item, company_abbr) for item in docs]
